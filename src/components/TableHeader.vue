...
const processSelectedCircles = (level_id) => {
  ...
  if (hasAnySelectedNodeChildren) {
    // not the last level, fold selected nodes
    const nodeLevel = selectionTree.value.find((node) => filteredSelectedCircleIds.includes(node.id)).level;
    const sameLevelNodes = selectionTree.value.filter((node) => node.level === nodeLevel);
    ...
    // last level, fold parent nodes
    const parentIds = filteredSelectedCircleIds.map((id) => selectionTree.value.find((node) => node.children_id.includes(id)).id);
    const uniqueParentIds = [...new Set(parentIds)]
    const parentLevel = selectionTree.value.find((node) =>uniqueParentIds.includes(node.id)).level;
    const sameLevelParents = selectionTree.value.filter((node) => node.level === parentLevel);
