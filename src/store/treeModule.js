...
const actions = {
  ...
  addYearLayer({ state, dispatch, commit, rootState }, obj) {
    ...
    commit("UPDATE_LEVELS", addSplitLevel(state.levels, "Region_Year"));
    commit(
      "scatterPlot/UPDATE_COORDINATE_COLLECTION",
      addCoordinate(rootState.scatterPlot.coordinateCollection, yearCoordinate),
      { root: true }
    );
  ...
  deleteNodes({ state }, deleteIds) {
    ...
    const parent_node = selectionTree.find((node) =>
      node.children_id.includes(id)
    );
  ...
  moveNodeToTop({ state, commit, dispatch }, { level, nodeId }) {
    ...
    // 2. 构造 sequence
    const sequence = [1, nodeId]; // 第一位必须是 id=1，第二位是拖拽的节点
